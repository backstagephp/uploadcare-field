<div class="space-y-4" wire:key="media-grid-picker-{{ $fieldName }}">
    <div class="flex items-center justify-between gap-4">
        <div class="flex-1">
            <input
                ...
                class="w-full px-3 py-2 text-sm bg-white text-gray-900 border border-gray-300 rounded-md dark:bg-gray-800 dark:border-gray-600 dark:text-gray-300 focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
            />
        </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 max-h-96 overflow-y-auto">
        @forelse($this->mediaItems as $media)
            @php
                $isSelected = $multiple 
                    ? in_array($media['id'], $selectedMediaIds) 
                    : ($selectedMediaId === $media['id']);
            @endphp
            <div 
                wire:key="media-item-{{ $media['id'] }}"
                wire:click="selectMedia({{ json_encode($media) }})"
                class="relative group cursor-pointer rounded-lg border-2 transition-all duration-200 hover:shadow-md {{ $isSelected ? 'border-blue-500 ring-2 ring-blue-200' : 'border-gray-200 hover:border-gray-300 dark:border-gray-700 dark:hover:border-gray-600' }}"
            >
